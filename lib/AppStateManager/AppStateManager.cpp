...
AppStateManager::AppStateManager(ButtonManager* buttons, SettingsManager* settings, 
    ...
    : buttons(buttons), settings(settings), display(display), effects(effects),
      menu(menu), clock(clock), currentState(SHOW_TIME), 
      blockMenuReentry(false), enterPressTime(0), wasPressed(false) {
}
